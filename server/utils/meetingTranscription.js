...
async function generateMeetingSummaryFromTranscript(transcriptRaw, meeting, options = {}) {
  ...
  const revisionQuestionsSchemaHint = isEducation
    ? '"revisionQuestions": "REQUIRED. One string only. Put 4–6 numbered lines (1. ... 2. ...). Mix exam-style types across the set: Define…, Differentiate… (or Compare…), Explain…, Short answer / list / apply. Each must be answerable only from this lecture. No new topics. If the class was very short, use at least 3 questions. Formatting: light Markdown OK; do not over-bold.",'
    : '';
  ...
  const userElaborationRules = isEducation
    ? `- Mirror the instructor’s progression: STRUCTURED NOTES bullets follow class order; DETAILED EXPLANATION then unpacks the same ideas with full reasoning and examples—recover middle steps of arguments the class heard.\n` +
      `- When an explanation was long, use several short paragraphs in DETAILED EXPLANATION (not one huge paragraph); optional ### mini-headings only where they improve skim-reading.\n` +
      `- keyPoints must stay one line each—never move paragraph-level explanation into keyPoints.\n` +
      `- Keep STRUCTURED NOTES bullets tight (one idea per line); do not stuff long explanations into bullets.\n` +
